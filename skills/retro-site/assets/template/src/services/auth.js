...
export function createUser(ctx, { username, password, passwordHash, role = 'user', city = '', createdAt }) {
  const { db } = ctx;
  return Number(db.prepare('INSERT INTO users (username, password_hash, role, city, created_at) VALUES (?, ?, ?, ?, ?)')
    .run(username, passwordHash || hashPassword(password), role, city, createdAt ?? now()).lastInsertRowid);
}
...
export function ensureAdmin(ctx) {
  ...
  if (adminUsername && adminPassword) {
    ...
    if (!existing) {
      createUser(ctx, { username: adminUsername, password: adminPassword, role: 'admin' });
      if (!ctx.config.isTest) console.log(`[__SLUG__] 已创建站长账号：${adminUsername}`);
    ...
    console.warn('[__SLUG__] 尚无站长账号：请设置 ADMIN_USERNAME / ADMIN_PASSWORD 环境变量，或运行 npm run create-admin');
  }
}
