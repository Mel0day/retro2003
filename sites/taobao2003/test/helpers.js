...
export const ADMIN = { username: '淘宝小二', password: 'admin123456789' };
export const PASSWORD = 'pass1234';

export async function startApp(env = {}) {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'taobao2003-test-'));
  ...
  const api = {
    ...
    // 直接建宝贝，返回 id
    createItem: (sellerId, over = {}) => {
      const it = {
        cat: '数码', title: '测试宝贝 一二三四', descr: '测试用', tag: '', price_cents: 10000, orig_cents: 0,
        stock: 5, city: '浙江 杭州', detail: '', images: '[]', status: 'onsale', ...over,
      };
      const t = now();
      return Number(ctx.db.prepare(`INSERT INTO items (seller_id, cat, title, descr, tag, price_cents, orig_cents, stock, city, detail, images, status, search_text, listed_at, created_at, updated_at)
        VALUES (@seller_id, @cat, @title, @descr, @tag, @price_cents, @orig_cents, @stock, @city, @detail, @images, @status, @search_text, @t, @t, @t)`)
        .run({ ...it, seller_id: sellerId, search_text: buildSearchText({ ...it, sellerName: ctx.db.prepare('SELECT username FROM users WHERE id = ?').get(sellerId).username }), t }).lastInsertRowid);
    },
    // 登录并返回客户端
